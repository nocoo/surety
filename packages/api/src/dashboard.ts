...
export async function getDashboardData(repos: AllRepos) {
	...
	const premiumByMember = Array.from(memberPremiumMap.values()).sort(
	...
	next12Months.forEach((month) => {
		renewalByCategoryMap.set(month, {});
	});
	...
	const renewalTimeline = {
		data: next12Months.map((month) => ({
			month,
			label: `${parseInt(month.split("-")[1] ?? "1", 10)}月`,
	...
	expiryPeriods.forEach((period) => {
		expiryByCategoryMap.set(period.key, {});
	});
