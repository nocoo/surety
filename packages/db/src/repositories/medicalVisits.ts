...
import { type MedicalVisit, medicalVisits, type NewMedicalVisit } from "../schema";
...
export function createMedicalVisitsRepo(dbInstance: DbInstance) {
	return {
		...
		async findById(id: number): Promise<MedicalVisit | undefined> {
			return await dbInstance.select().from(medicalVisits).where(eq(medicalVisits.id, id)).get();
		},
