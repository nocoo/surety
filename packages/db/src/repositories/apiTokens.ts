...
export function createApiTokensRepo(dbInstance: DbInstance) {
	return {
		...
		async findById(id: number): Promise<ApiToken | undefined> {
			return await dbInstance.select().from(apiTokens).where(eq(apiTokens.id, id)).get();
		},

		async revoke(id: number): Promise<boolean> {
			const rows = await dbInstance.delete(apiTokens).where(eq(apiTokens.id, id)).returning().all();
