...
export function generatePaymentRecords(
	...
	const hasExplicitAnchor = input.anchorDate != null && input.anchorDate !== firstDueDate;
	...
	const maxPeriods = input.totalPayments ?? (cutoffDate !== null ? 1200 : 1);
	...
	for (let periodNumber = 1; periodNumber <= maxPeriods; periodNumber++) {
		...
		const dueDate = periodDate(aYear, aMonth, aDay, offsetFromAnchor, paymentFrequency);
