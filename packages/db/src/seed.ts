...
import type { NewAsset, NewBeneficiary, NewMember, NewPolicy } from "./schema";
...
export const familyMembers: NewMember[] = [
	...
	},
	{
		...
		relation: "Parent",
		gender: "M",
		birthDate: "1956-02-28",
		idCard: "110101195602280066",
		phone: "13800138006",
	},
	{
		name: "刘桂芳",
...
export const familyAssets: (Omit<NewAsset, "ownerId"> & { ownerName: string })[] = [
	{
		...
		details: JSON.stringify({
			address: "北京市朝阳区建国路88号院1号楼1单元101",
			area: 120,
			purchaseYear: 2020,
			value: 8000000,
		}),
...
export async function seedDatabase(repos?: AllRepos, dbInstance?: DbInstance): Promise<SeedResult> {
	...
	const doSeed = async (): Promise<SeedResult> => {
		...
		await r.settings.setJson("riskTolerance", {
			level: "moderate",
