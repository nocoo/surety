...
import type { NewMember, NewAsset, NewPolicy, NewBeneficiary } from "../src/db/schema";

const familyMembers: NewMember[] = [
...
const familyAssets: (Omit<NewAsset, "ownerId"> & { ownerName: string })[] = [
  {
    type: "RealEstate",
    name: "朝阳区住宅",
...
async function seed() {
  ...
  for (const member of familyMembers) {
    ...
    console.log(`  + ${member.name} (${member.relation})`);
  }
  ...
  for (const asset of familyAssets) {
    ...
    console.log(`  + ${asset.name} (${asset.identifier})`);
  }
  ...
  for (const seed of policySeedData) {
    ...
    console.log(`  + ${seed.policy.productName} (${seed.policy.policyNumber})`);
  ...
  console.log("  + annualIncome, emergencyFundMonths, riskTolerance");
