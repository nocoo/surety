...
app.post("/api/doctors", async (c) => {
	...
	if (!body.name || !body.hospitalId || !body.department)
		return c.json({ error: "name, hospitalId, and department are required" }, 400);
...
app.delete("/api/doctors/:id", async (c) => {
	...
	if (visits.length > 0)
		return c.json({ error: `该医生关联了 ${visits.length} 条就诊记录，无法删除` }, 409);
