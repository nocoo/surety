...
app.post("/api/policies", async (c) => {
	...
	if (isTerminalStatus(body.status)) {
		return c.json(
			{
				error:
					"Cannot create a policy in a terminated state — use POST /api/policies/:id/terminate after creation",
			},
			400,
		);
	}
	...
	if (bodyHasStatusMetadata(body)) {
		return c.json(
			{
				error:
					"Cannot set termination or planned-surrender metadata on create — use the dedicated transition endpoints after creation",
			},
			400,
		);
	}
...
app.get("/api/policies/:id", async (c) => {
	...
	if (Number.isNaN(policyId)) return c.json({ error: "Invalid id" }, 400);
...
app.put("/api/policies/:id", async (c) => {
	...
	if (!isReactivation) {
		...
		if (bodyHasStatusMetadata(body)) {
			return c.json(
				{ error: "Cannot modify status metadata via PUT — use the dedicated transition endpoints" },
				400,
			);
		}
	}
...
app.post("/api/policies/:id/terminate", async (c) => {
	...
	if (isTerminalStatus(existing.status) && existing.status !== status) {
		return c.json(
			{ error: "Cannot transition between terminal statuses; reactivate to Active first" },
			400,
		);
	}
...
app.get("/api/policies/:id/beneficiaries", async (c) => {
	const repos = c.get("repos");
	const policyId = parseInt(c.req.param("id"), 10);
	if (Number.isNaN(policyId)) return c.json({ error: "Invalid policy ID" }, 400);
	...
	return c.json(
		records.map((b) => ({
			...
			memberId: b.memberId,
			name: b.memberId
				? (memberMap.get(b.memberId) ?? b.externalName ?? "未知")
				: (b.externalName ?? "未知"),
...
app.get("/api/policies/:id/payments", async (c) => {
	...
	return c.json(
		payments.sort(
			(a: { periodNumber: number }, b: { periodNumber: number }) => b.periodNumber - a.periodNumber,
		),
	);
});
...
app.put("/api/policies/:id/payments/:paymentId", async (c) => {
	...
	if (Number.isNaN(policyId) || Number.isNaN(paymentIdNum))
	...
	if (body.periodNumber !== undefined && body.periodNumber !== existing.periodNumber) {
		...
		const duplicate = policyPayments.find(
			(p: { periodNumber: number; id: number }) =>
				p.periodNumber === body.periodNumber && p.id !== paymentIdNum,
		);
...
app.post("/api/policies/:id/payments/generate", async (c) => {
	...
	const existingPeriodNumbers = new Set(
		existingPayments.map((p: { periodNumber: number }) => p.periodNumber),
	);
	...
	// Period numbering anchors on effectiveDate (period 1 = effective day);
	// the schedule itself is anchored on the user-recorded nextDueDate so
	// billing-day drift is honored (typical case: a policy that took effect
	// 2022-12-29 but the user pays on 6/29 every year). Past periods between
	// effectiveDate and nextDueDate are backfilled as Pending, so a long-
	// running policy with no payment records doesn't silently produce only
	// the next single period (the bug哥 reported on policy #24).
	const firstDueDate = policy.effectiveDate;
	const anchorDate = policy.nextDueDate ?? null;
...
app.get("/api/policies/:id/coverage-items/:itemId", async (c) => {
	...
	if (Number.isNaN(policyId) || Number.isNaN(itemId)) return c.json({ error: "Invalid ID" }, 400);
...
app.put("/api/policies/:id/coverage-items/:itemId", async (c) => {
	...
	if (!existing || existing.policyId !== policyId)
		return c.json({ error: "Coverage item not found" }, 404);
	const body = await c.req.json();
...
app.post("/api/policies/:id/attachments", async (c) => {
	...
	let attachment: Awaited<ReturnType<typeof repos.attachments.create>>;
	try {
...
app.get("/api/policies/:id/attachments/:attachmentId", async (c) => {
	...
	if (Number.isNaN(policyId) || Number.isNaN(attId)) return c.json({ error: "Invalid ID" }, 400);
...
app.get("/api/policies/:id/attachments/:attachmentId/file", async (c) => {
	...
	const utf8Name = encodeURIComponent(attachment.filename).replace(
		/['()]/g,
