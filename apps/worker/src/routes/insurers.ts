...
app.get("/api/insurers", async (c) => {
	...
	return c.json(
		insurers.map(
			(i: { id: number; name: string; phone: string | null; website: string | null }) => ({
				id: i.id,
				name: i.name,
				phone: i.phone,
				website: i.website,
...
app.post("/api/insurers", async (c) => {
	...
	return c.json(
		{ id: insurer.id, name: insurer.name, phone: insurer.phone, website: insurer.website },
		201,
	);
});
...
app.put("/api/insurers/:id", async (c) => {
	...
	if (body.name) {
		...
		if (existing && existing.id !== id)
			return c.json({ error: "Insurer with this name already exists" }, 409);
	}
	const updated = await repos.insurers.update(id, {
		name: body.name,
		phone: body.phone,
