...
app.get("/api/renewal-calendar", async (c) => {
  ...
  const activePolicies = policies.filter((p: { status: string; expiryDate: string | null }) => isEffectivelyActive(p.status as PolicyDbStatus, p.expiryDate));
  const memberMap = new Map(members.map((m: { id: number; name: string }) => [m.id, m.name]));

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const policiesForRenewal: PolicyForRenewal[] = activePolicies.map((p: any) => ({
    id: p.id, productName: p.productName, category: p.category,
    subCategory: p.subCategory, premium: p.premium, paymentFrequency: p.paymentFrequency,
