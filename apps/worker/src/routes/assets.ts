...
app.get("/api/assets", async (c) => {
  ...
  return c.json(assets.map((a: any) => ({
    id: a.id, type: a.type, name: a.name, identifier: a.identifier,
    ownerId: a.ownerId, ownerName: a.ownerId ? memberMap.get(a.ownerId) ?? null : null,
    details: a.details,
  })));
});
...
app.post("/api/assets", async (c) => {
  ...
  if (!body.type || !body.name || !body.identifier) return c.json({ error: "type, name, and identifier are required" }, 400);
  const asset = await repos.assets.create({
    type: body.type, name: body.name, identifier: body.identifier,
    ownerId: body.ownerId ?? null, details: body.details ?? null,
  });
...
app.put("/api/assets/:id", async (c) => {
  ...
  const updated = await repos.assets.update(id, {
    type: body.type, name: body.name, identifier: body.identifier,
    ownerId: body.ownerId, details: body.details,
  });
...
app.delete("/api/assets/:id", async (c) => {
  ...
  if (linked.length > 0) return c.json({ error: `该资产关联了 ${linked.length} 份保单，无法删除` }, 409);
