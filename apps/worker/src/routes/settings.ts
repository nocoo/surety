...
app.get("/api/settings", async (c) => {
  ...
  return c.json(settings.filter((s: { key: string }) => !isLegacySensitiveKey(s.key)).map((s: { key: string; value: string }) => ({ key: s.key, value: s.value })));
});
...
app.post("/api/settings", async (c) => {
  ...
  if (isLegacySensitiveKey(String(body.key))) return c.json({ error: "Cannot modify legacy sensitive settings" }, 403);
...
app.get("/api/settings/backy/history", async (c) => {
  ...
  const webhookUrl = (await repos.settings.get("backy.webhookUrl")) ?? "";
  const apiKey = (await repos.settings.get("backy.apiKey")) ?? "";
  if (!webhookUrl || !apiKey) return c.json({ error: "Backy webhook URL and API key must be configured first" }, 400);
  try {
    const res = await fetch(webhookUrl, { method: "GET", headers: { Authorization: `Bearer ${apiKey}` } });
    if (!res.ok) { const text = await res.text().catch(() => ""); return c.json({ error: text || `HTTP ${res.status}` }, 502); }
...
app.post("/api/settings/backy/test", async (c) => {
  ...
  if (!webhookUrl || !apiKey) return c.json({ error: "Backy webhook URL and API key must be configured first" }, 400);
  try {
    const res = await fetch(webhookUrl, { method: "HEAD", headers: { Authorization: `Bearer ${apiKey}` } });
    return c.json({ success: res.ok, status: res.status });
  } catch (err) {
    return c.json({ success: false, status: 0, error: err instanceof Error ? err.message : String(err) }, 502);
  }
});
...
app.post("/api/settings/backy/push", async (c) => {
  ...
  const requestMeta = {
    url: webhookUrl, method: "POST" as const, environment: "prod", tag, fileName: filename, fileSizeBytes: json.length,
    backupStats: { members: d.members.length, insurers: d.insurers.length, assets: d.assets.length, policies: d.policies.length, beneficiaries: d.beneficiaries.length, payments: d.payments.length, cashValues: d.cashValues.length, coverageItems: d.coverageItems.length, settings: d.settings.length },
  };

  try {
    const res = await fetch(webhookUrl, { method: "POST", headers: { Authorization: `Bearer ${apiKey}` }, body: form });
    let body: unknown;
    const text = await res.text().catch(() => "");
    try { body = JSON.parse(text); } catch { body = text || null; }
    if (!res.ok) return c.json({ success: false, error: `Backy returned HTTP ${res.status}`, request: requestMeta, response: { status: res.status, body }, durationMs: Date.now() - start }, 502);
    return c.json({ success: true, request: requestMeta, response: { status: res.status, body }, durationMs: Date.now() - start });
  } catch (err) {
    return c.json({ success: false, error: err instanceof Error ? err.message : String(err), request: requestMeta, response: { status: 0, body: null }, durationMs: Date.now() - start }, 502);
  }
});
...
app.get("/api/settings/:key", async (c) => {
  ...
  if (isLegacySensitiveKey(key)) return c.json({ error: "Cannot access legacy sensitive settings" }, 403);
