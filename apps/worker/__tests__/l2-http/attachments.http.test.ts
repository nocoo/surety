...
import { BASE_URL, httpJson } from "./setup";
...
describe("L2-HTTP: attachments via real R2 binding", () => {
	test("upload → list → get metadata → download bytes → delete", async () => {
		...
		const upRes = await fetch(`${BASE_URL}/api/policies/${policyId}/attachments`, {
			method: "POST",
			body: form,
		});
		...
		const list = await httpJson<Attachment[]>("GET", `/api/policies/${policyId}/attachments`);
		...
		const fileRes = await fetch(`${BASE_URL}/api/policies/${policyId}/attachments/${att.id}/file`);
		...
		const del = await httpJson("DELETE", `/api/policies/${policyId}/attachments/${att.id}`);
		expect(del.status).toBe(204);

		const after = await httpJson("GET", `/api/policies/${policyId}/attachments/${att.id}`);
	...
	test("rejects non-PDF magic bytes even when content-type claims PDF", async () => {
		...
		const res = await fetch(`${BASE_URL}/api/policies/${policyId}/attachments`, {
			method: "POST",
			body: form,
		});
