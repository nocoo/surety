...
export function definePoliciesCommand(
  factory: () => ApiClient = buildClient,
) {
  ...
  const update = defineCommand({
    ...
    async run({ args }) {
      const body = requireJsonInput(args);
      const row = await client().put<Policy>(
        `/api/policies/${args.id}`,
        body,
      );
      emitRecord(row, summarizePolicy, { full: Boolean(args.full) });
    },
  ...
  const paymentsLs = defineCommand({
    ...
    args: { ...policyIdArg, ...fullArg },
    async run({ args }) {
      const rows = await client().get<Payment[]>(
        `/api/policies/${args.id}/payments`,
      );
  ...
  const paymentsAdd = defineCommand({
    ...
    async run({ args }) {
      const body = requireJsonInput(args);
      const row = await client().post<Payment>(
        `/api/policies/${args.id}/payments`,
        body,
      );
  ...
  const beneficiariesLs = defineCommand({
    ...
    args: { ...policyIdArg, ...fullArg },
    async run({ args }) {
      const rows = await client().get<Beneficiary[]>(
        `/api/policies/${args.id}/beneficiaries`,
      );
  ...
  const coverageLs = defineCommand({
    ...
    args: { ...policyIdArg, ...fullArg },
    async run({ args }) {
      const rows = await client().get<CoverageItem[]>(
  ...
  const attachmentsLs = defineCommand({
    ...
    args: { ...policyIdArg, ...fullArg },
    async run({ args }) {
      const rows = await client().get<Attachment[]>(
        `/api/policies/${args.id}/attachments`,
      );
