...
import { createConfig, resolveApiUrl, resolveLoginUrl, resolveToken } from "../config.js";
...
const login = defineCommand({
	...
	async run({ args }) {
		...
		const loginUrl = (args["login-url"] as string | undefined) ?? resolveLoginUrl(config);
		const apiUrl = (args["api-url"] as string | undefined) ?? resolveApiUrl(config);
		const timeoutSec = args.timeout ? Number.parseInt(String(args.timeout), 10) : 180;
...
const whoami = defineCommand({
	...
	async run() {
		...
		const me = await client.get<{ email: string; authenticated: boolean }>("/api/me");
