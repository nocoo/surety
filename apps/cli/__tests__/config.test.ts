...
describe("resolveLoginUrl", () => {
	test("env SURETY_LOGIN_URL wins over config", () => {
		...
		expect(resolveLoginUrl(cfg, env({ SURETY_LOGIN_URL: "https://env-login.example" }))).toBe(
			"https://env-login.example",
		);
	});
...
describe("resolveToken", () => {
	test("env SURETY_API_TOKEN wins over config", () => {
		...
		expect(resolveToken(cfg, env({ SURETY_API_TOKEN: "from-env" }))).toBe("from-env");
	});
