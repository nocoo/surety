...
import { Building2, Check, Copy, Phone, Users } from "lucide-react";
...
import { buildCoverageClipboardText, buildEmergencyContacts } from "./emergency";

const breadcrumbs = [{ label: "保障速查" }];

export default function CoverageLookupPage() {
	const [searchParams, setSearchParams] = useSearchParams();
	const initial = useMemo(() => readCoverageDeepLink(searchParams), [searchParams]);
	...
	const filteredGroups: CategoryGroup[] = useMemo(() => {
		...
		const activeOnly = data.categoryGroups.flatMap((g) => g.policies.filter((p) => p.isActive));
	...
	const inactiveCount = useMemo(() => {
		if (!data) return 0;
		return data.categoryGroups.flatMap((g) => g.policies).filter((p) => !p.isActive).length;
	}, [data]);
            ...
            offer fingers-not-thumbs hit targets. */}
				...
				>
					...
					>
						...
						家庭成员
						{hasMembers && <span className="text-sm opacity-70">({data.members.length})</span>}
					</button>
					<button
						type="button"
						role="tab"
					...
					>
						...
						资产
						{hasAssets && <span className="text-sm opacity-70">({data.assets.length})</span>}
					</button>
				</div>
				...
				{emergencyContacts.length > 0 && (
					...
					>
						<header className="mb-3 flex items-center gap-2">
							...
							<span className="text-xs text-muted-foreground">· 保险公司客服，可直接拨打</span>
						</header>
						<ul className="grid gap-2 sm:grid-cols-2">
							{emergencyContacts.map((c) => (
								<li key={`${c.insurerName}-${c.phone}`}>
									...
									>
										<span className="text-sm font-medium truncate">{c.insurerName}</span>
