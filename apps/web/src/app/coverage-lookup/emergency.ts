 ...
 */
export function buildEmergencyContacts(groups: readonly CategoryGroup[]): EmergencyContact[] {
...
export function buildCoverageClipboardText(
	...
	for (const group of visible) {
		...
		lines.push("");
		lines.push(
			`▎${group.categoryLabel}（${group.policies.length} 份 · 总保额 ${formatCurrency(groupTotal)}）`,
		);
		for (const policy of group.policies) {
