...
import { formatDaysFromToday, getDaysFromToday } from "@surety/db/lib/date-utils";
				...
				<div>
					...
					<div className="mt-1 flex items-center gap-2">
						<Avatar className="h-6 w-6">
							<AvatarFallback
								className={cn("text-xs text-white", getAvatarColor(policy.insuredName ?? ""))}
							>
...
export default function PoliciesPage() {
	...
	const [filterInsured, setFilterInsured] = usePersistedState<string>(
		"surety-filter-insured",
		"all",
	);
	const [filterApplicant, setFilterApplicant] = usePersistedState<string>(
		"surety-filter-applicant",
		"all",
	);
	const [filterCategory, setFilterCategory] = usePersistedState<string>(
	...
	const assetNames = useMemo(() => {
		const names = new Set(
			policies.filter((p) => p.insuredAssetName).map((p) => p.insuredAssetName as string),
		);
		return Array.from(names).sort();
	}, [policies]);
		...
		<AppShell breadcrumbs={[{ label: "保单" }]}>
			<div className="space-y-6">
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							...
							{countActiveFilters(filters) > 0 && ` (已筛选，共 ${policies.length} 份)`}
						</p>
					</div>
															...
															<div className="flex items-center gap-2">
																<Avatar className="h-6 w-6">
																	<AvatarFallback
																		className={cn(
																			"text-xs text-white",
																			getAvatarColor(policy.applicantName ?? ""),
																		)}
																	>
															...
															<div className="flex items-center gap-2">
																<Avatar className="h-6 w-6">
																	<AvatarFallback
																		className={cn(
																			"text-xs text-white",
																			getAvatarColor(policy.insuredName ?? ""),
																		)}
																	>
																		{policy.insuredName?.[0] ?? "?"}
																	</AvatarFallback>
															...
															{policy.attachmentCount > 0 && (
																<TooltipProvider delayDuration={0}>
																	<Tooltip>
																		...
																		<TooltipContent>
																			<p className="text-xs">
																				{policy.attachmentCount} 个附件，点击预览
																			</p>
																		</TooltipContent>
																	</Tooltip>
																</TooltipProvider>
				...
				{(viewMode === "byCategory" || viewMode === "byInsured") && policies.length > 0 && (
					<div className="space-y-6">
						{(viewMode === "byCategory" ? policiesByCategory : policiesByInsured).map(
							([groupKey, groupPolicies]) => {
								const groupLabel =
									viewMode === "byCategory" ? getCategoryConfig(groupKey).label : groupKey;
											...
											<div className="flex items-center gap-3">
												{viewMode === "byInsured" && (
													<Avatar className="h-8 w-8">
														<AvatarFallback
															className={cn("text-sm text-white", getAvatarColor(groupKey))}
														>
															{groupKey[0]}
														</AvatarFallback>
													</Avatar>
													...
													>
														...
														<div className="flex items-center gap-6">
															<div className="text-right">
																...
																<div className="text-xs text-muted-foreground">
																	{formatCurrency(policy.premium)}/年
																</div>
															</div>
															<div className="flex items-center gap-1">
																<Button
			...
			<PolicySheet open={sheetOpen} onOpenChange={setSheetOpen} onSuccess={fetchPolicies} />
