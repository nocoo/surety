...
function HospitalForm({
	...
	const [formData, setFormData] = useState<HospitalFormData>(() => createFormData(hospital));
	const [isSubmitting, setIsSubmitting] = useState(false);
	const [error, setError] = useState<string | null>(null);

	const handleChange = <K extends keyof HospitalFormData>(field: K, value: HospitalFormData[K]) => {
	...
	return (
		<>
			<SheetHeader>
				...
				<SheetDescription>{isEditing ? "修改医院信息" : "添加新的医院"}</SheetDescription>
			</SheetHeader>
				...
				<div className="space-y-4">
					...
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							...
							<p className="text-xs text-muted-foreground">公立医院通常医保报销比例更高</p>
						</div>
						<Switch
							id="isPublic"
...
export function HospitalSheet({ open, onOpenChange, hospital, onSuccess }: HospitalSheetProps) {
	return (
