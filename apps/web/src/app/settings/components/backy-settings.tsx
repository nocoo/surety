import { AlertTriangle, Cloud, History, Loader2, Plug, RefreshCw, Save, Send } from "lucide-react";
...
export function BackySettings() {
	...
	const [backyTestResult, setBackyTestResult] = useState<{
		success: boolean;
		status: number;
		error?: string;
	} | null>(null);
		...
		<div className="rounded-card bg-secondary p-6">
			<div className="flex items-center gap-3 mb-4">
				...
				<div className="flex items-center gap-2">
					...
					<Badge
						variant={backySettings?.environment === "prod" ? "success" : "warning"}
						className="ml-1"
					>
					...
					>
						{backyTestResult.success ? (
							...
							<>
								<AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
								<p>
									连接失败{backyTestResult.status ? ` (HTTP ${backyTestResult.status})` : ""}
									{backyTestResult.error ? `: ${backyTestResult.error}` : ""}
								</p>
							</>
						)}
					</Notice>
					...
					<Notice variant={backyPushResult.success ? "success" : "destructive"}>
						{backyPushResult.success ? (
							<div className="space-y-2">
								<p className="font-medium">推送成功 ({backyPushResult.durationMs}ms)</p>
								{backyPushResult.request && (
									<div className="text-xs space-y-1">
										<p>Tag: {backyPushResult.request.tag}</p>
										<p>
											文件: {backyPushResult.request.fileName} (
											{formatFileSize(backyPushResult.request.fileSizeBytes)})
										</p>
								...
								<div>
									...
									{backyPushResult.response && (
										<p className="text-xs mt-1">
											HTTP {backyPushResult.response.status}:{" "}
											{JSON.stringify(backyPushResult.response.body)}
										</p>
									)}
								</div>
							</div>
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								{backyHistory && <Badge variant="secondary">{backyHistory.total_backups} 份</Badge>}
							</div>
							<Button
								variant="ghost"
								size="sm"
						...
						{backyHistory && backyHistory.recent_backups.length > 0 ? (
							<div className="grid gap-2 sm:grid-cols-2">
								{backyHistory.recent_backups.map((entry) => (
									<div
										key={entry.id}
										className="rounded-widget border bg-muted/50 p-3 text-xs space-y-1"
									>
										<div className="flex items-center justify-between">
										...
										<p className="text-muted-foreground truncate" title={entry.tag}>
											{entry.tag}
										</p>
