...
import { renderPolicyStatusBadges, statusConfig, statusStripeClass } from "@/lib/constants/policy";
...
describe("renderPolicyStatusBadges", () => {
	...
	it("returns only the primary badge when plannedSurrenderAt is null", () => {
		expect(renderPolicyStatusBadges({ status: "Active", plannedSurrenderAt: null })).toEqual([
			{ label: "生效中", variant: "success" },
		]);
	});
