...
import {
  readCoverageDeepLink,
  selectorKindForDeepLink,
} from "@/app/coverage-lookup/deep-link";
...
describe("selectorKindForDeepLink", () => {
  ...
  it("URL with no params → member selector (default landing)", () => {
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("")))).toBe(
      "member-selector",
    );
  });
  ...
  it("clicking the asset tab without picking → asset selector visible", () => {
    ...
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("asset")))).toBe(
      "asset-selector",
    );
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("asset=")))).toBe(
      "asset-selector",
  ...
  it("?asset=N keeps the asset selector visible", () => {
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("asset=42")))).toBe(
      "asset-selector",
    );
  });

  it("?member / ?member=N → member selector", () => {
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("member")))).toBe(
      "member-selector",
    );
    expect(selectorKindForDeepLink(readCoverageDeepLink(p("member=42")))).toBe(
      "member-selector",
    );
  });
});
