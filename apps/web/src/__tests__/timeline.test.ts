...
describe("buildTimeline — termination", () => {
  ...
  it("inserts a termination milestone with the correct label, date and ordering", () => {
    ...
    const surr = buildTimeline(
      basePolicy({ status: "Surrendered", terminatedAt: "2026-06-15" }),
    );
    expect(surr.some((e) => e.label === "退保")).toBe(true);
    const lapsed = buildTimeline(
      basePolicy({ status: "Lapsed", terminatedAt: "2026-06-15" }),
    );
