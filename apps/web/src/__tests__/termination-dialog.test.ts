...
describe("validateTerminationForm", () => {
	...
	it("requires terminatedAt", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "" })).toMatch(/请填写/);
	});

	it("rejects malformed terminatedAt", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "not-a-date" })).toMatch(/格式/);
	});

	it("rejects terminatedAt earlier than effectiveDate", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "2025-12-31" })).toMatch(/生效日/);
	});

	it("rejects terminatedAt later than today", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "2026-07-01" })).toMatch(/今天/);
	});
	...
	it("accepts terminatedAt equal to effectiveDate (boundary in)", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "2026-01-01" })).toBeNull();
	});

	it("accepts terminatedAt equal to today (boundary in)", () => {
		expect(validateTerminationForm({ ...BASE, terminatedAt: "2026-06-20" })).toBeNull();
	});
});
