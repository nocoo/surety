...
import { isItemActive, NAV_GROUPS, shouldGroupBeOpenOnMount } from "@/lib/navigation";
...
describe("shouldGroupBeOpenOnMount", () => {
	...
	it("defaults to open when defaultOpen is undefined", () => {
		expect(shouldGroupBeOpenOnMount({ items: [{ href: "/x" }] }, "/y")).toBe(true);
	});

	it("honours an explicit defaultOpen=false when no item matches", () => {
		expect(
			shouldGroupBeOpenOnMount({ items: [{ href: "/cli" }], defaultOpen: false }, "/policies"),
		).toBe(false);
	});
