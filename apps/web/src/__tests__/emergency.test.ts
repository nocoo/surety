...
describe("buildEmergencyContacts", () => {
	...
	it("skips inactive policies entirely", () => {
		const g = group("定期寿", [makePolicy({ id: 1, isActive: false })]);
...
describe("buildCoverageClipboardText", () => {
	...
	it("groups by category and emits a grand total", () => {
		const groups: CategoryGroup[] = [
			group("定期寿", [makePolicy({ id: 1, productName: "终身寿", sumAssured: 1_000_000 })]),
			group("医疗险", [makePolicy({ id: 2, productName: "百万医疗", sumAssured: 5_000_000 })]),
		];
