...
function PaymentForm({
	...
	const canSave = form.periodNumber && form.dueDate && form.amount && Number(form.amount) > 0;
		...
		<div className="space-y-3 rounded-lg border bg-background p-3">
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<div className="space-y-1.5">
					...
					<Select
						value={form.status}
						onValueChange={(v) => onChange({ ...form, status: v as "Pending" | "Paid" })}
					>
...
export function PaymentsSection({
	...
	const liveRows = payments.filter((p) => !isObsoletedByTermination(p, policyTerminatedAt));
	const obsoletedRows = payments.filter((p) => isObsoletedByTermination(p, policyTerminatedAt));
	const sortedLive = [...liveRows].sort((a, b) => a.periodNumber - b.periodNumber);
	const sortedObsoleted = [...obsoletedRows].sort((a, b) => a.periodNumber - b.periodNumber);
	...
	const startEdit = (payment: Payment) => {
		...
		setEditingForm(isTerminated ? paymentToFormForTerminatedEdit(payment) : paymentToForm(payment));
		setAddingForm(null);
	};
	...
	const handleAdd = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				setResultMessage((err as { error?: string } | null)?.error ?? `添加失败 (${res.status})`);
			}
		} catch {
	...
	const handleUpdate = async () => {
		...
		try {
			...
			const res = await fetch(`/api/policies/${policyId}/payments/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(body),
			});
			if (res.ok) {
				...
				onPaymentsChange(payments.map((p) => (p.id === editingId ? updated : p)));
				setEditingId(null);
			} else {
				const err = await res.json().catch(() => null);
				setResultMessage((err as { error?: string } | null)?.error ?? `修改失败 (${res.status})`);
			}
		} catch {
	...
	const handleMarkPaid = async (payment: Payment) => {
		...
		setResultMessage(null);
		try {
			const res = await fetch(`/api/policies/${policyId}/payments/${payment.id}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					status: "Paid",
			...
			if (res.ok) {
				const updated = (await res.json()) as Payment;
				onPaymentsChange(payments.map((p) => (p.id === payment.id ? updated : p)));
			} else {
				const err = await res.json().catch(() => null);
				setResultMessage((err as { error?: string } | null)?.error ?? `标记失败 (${res.status})`);
			}
		} catch {
	...
	const handleDelete = async (paymentId: number) => {
		setResultMessage(null);
		try {
			const res = await fetch(`/api/policies/${policyId}/payments/${paymentId}`, {
				method: "DELETE",
			});
			if (res.ok) {
				...
				const err = await res.json().catch(() => null);
				setResultMessage((err as { error?: string } | null)?.error ?? `删除失败 (${res.status})`);
			}
		} catch {
	...
	const handleGenerate = async () => {
		...
		setResultMessage(null);
		try {
			const res = await fetch(`/api/policies/${policyId}/payments/generate`, { method: "POST" });
			if (!res.ok) {
				const body = await res.json().catch(() => null);
				setResultMessage((body as { error?: string } | null)?.error ?? `生成失败 (${res.status})`);
				return;
			}
		...
		<div>
			...
			{liveRows.length > 0 && (
				<div className="mb-3 rounded-lg bg-muted/30 px-3 py-2 text-sm">
					...
					期 &mdash; <span className="font-medium">{formatCurrencyFull(paidAmount)}</span> /{" "}
			...
			{resultMessage && (
				<p
					className={cn(
						"mb-2 text-sm",
						resultMessage.startsWith("成功") ? "text-success-text" : "text-destructive-text",
					)}
				>
					{resultMessage}
				</p>
			...
			>
				<AlertDialogContent size="sm">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>确定删除该缴费记录吗？此操作不可撤销。</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
