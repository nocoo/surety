...
export function buildPlannedSurrenderPayload(input: {
	...
	return {
		...
		plannedSurrenderNote: input.plannedSurrenderNote.length > 0 ? input.plannedSurrenderNote : null,
	};
}
...
export function PlannedSurrenderDialog({
	...
	const [plannedSurrenderAt, setPlannedSurrenderAt] = useState(policy.plannedSurrenderAt ?? "");
	...
	async function submit(payload: object) {
		...
		setError(null);
		try {
			const res = await fetch(`/api/policies/${policy.id}/planned-surrender`, {
				method: "PUT",
				credentials: "include",
