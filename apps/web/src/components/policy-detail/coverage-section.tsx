...
function formToPayload(form: CoverageFormData, sortOrder: number) {
	return {
		...
		coveragePercent: form.coveragePercent ? Number(form.coveragePercent) : null,
...
function CoverageForm({
	...
	const update = (field: keyof CoverageFormData, value: string | boolean) =>
		...
		<div className="space-y-3 rounded-lg border bg-background p-3">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					<Switch checked={form.isOptional} onCheckedChange={(v) => update("isOptional", v)} />
...
export function CoverageSection({ policyId, items, onItemsChange }: CoverageSectionProps) {
	...
	const handleAdd = async () => {
		if (!addingForm?.name.trim()) return;
		setSaving(true);
		...
		try {
			...
			if (res.ok) {
				...
				setErrorMessage((err as { error?: string } | null)?.error ?? `添加失败 (${res.status})`);
			}
		} catch {
	...
	const handleUpdate = async () => {
		...
		try {
			...
			const res = await fetch(`/api/policies/${policyId}/coverage-items/${editingId}`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(formToPayload(editingForm, existing?.sortOrder ?? 0)),
			});
			if (res.ok) {
				...
				const err = await res.json().catch(() => null);
				setErrorMessage((err as { error?: string } | null)?.error ?? `修改失败 (${res.status})`);
			}
		} catch {
	...
	const handleDelete = async (itemId: number) => {
		...
		try {
			...
			if (res.ok) {
				...
				const err = await res.json().catch(() => null);
				setErrorMessage((err as { error?: string } | null)?.error ?? `删除失败 (${res.status})`);
			}
		} catch {
			...
			>
				<AlertDialogContent size="sm">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>确定删除该保障项目吗？此操作不可撤销。</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
