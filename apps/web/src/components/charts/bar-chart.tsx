...
function ChartTooltip({
	...
	return (
		<div className={TOOLTIP_STYLES.container}>
			...
			{data.count !== undefined && <p className={TOOLTIP_STYLES.value}>保单: {data.count} 份</p>}
		...
		<ChartCard title={title} icon={icon}>
			<ResponsiveContainer {...RESPONSIVE_CONTAINER_PROPS}>
				<BarChart data={data} layout="vertical" margin={{ left: 20, right: 20 }}>
					<XAxis type="number" tickFormatter={formatter} {...AXIS_CONFIG} />
					<YAxis type="category" dataKey="name" width={60} {...AXIS_CONFIG} />
					<Tooltip content={<ChartTooltip formatValue={formatValue} />} />
					<Bar dataKey={valueKey} fill={color} radius={BAR_RADIUS.horizontal} />
				</BarChart>
			</ResponsiveContainer>
		</ChartCard>
	);
		...
		<ChartCard title={title} icon={icon} height="h-[320px]">
			<ResponsiveContainer {...RESPONSIVE_CONTAINER_PROPS}>
				<BarChart data={data} layout="vertical" margin={{ left: 60, right: 20 }}>
					...
					<Bar
						dataKey="count"
						fill={CHART_COLORS.palette[2] as string}
	...
	return (
		<div className={TOOLTIP_STYLES.container}>
			...
			<p className="text-sm font-medium mt-1">
				合计{valueLabel}: {formatCurrency(total)}
			</p>
		</div>
	);
}
