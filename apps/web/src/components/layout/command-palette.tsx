...
interface PolicyLite { id: number; productName: string; insuredName: string; insurerName: string }
interface MemberLite { id: number; name: string; relation?: string }
interface HospitalLite { id: number; name: string }
interface DoctorLite { id: number; name: string; hospitalName?: string | null }
...
function PaletteContent({ onClose, active }: { onClose: () => void; active: boolean }) {
  ...
  const { data: policies } = useSWR<PolicyLite[]>(active ? "/api/policies" : null, fetchAPI, swrCfg);
  const { data: members } = useSWR<MemberLite[]>(active ? "/api/members" : null, fetchAPI, swrCfg);
  const { data: hospitals } = useSWR<HospitalLite[]>(active ? "/api/hospitals" : null, fetchAPI, swrCfg);
  ...
  const allCommands: PaletteCommand[] = useMemo(() => {
    return [
      ...
      ...(hospitals ?? []).map((h): PaletteCommand => ({
        id: `hospital-${h.id}`,
        label: h.name,
        hint: "医院",
        group: "医院",
...
function renderGroups(
  ...
  return filtered.map((cmd, i) => {
    ...
    return (
      <div key={cmd.id}>
        {header && (
          <div className="px-3 pt-2 pb-1 text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">
            {header}
          </div>
        )}
        <button
...
const NAV_COMMANDS: PaletteCommand[] = [
  { id: "nav-dashboard", label: "去仪表盘", group: "导航", icon: LayoutDashboard, perform: (n) => n("/") },
  { id: "nav-coverage", label: "去保障速查", group: "导航", icon: ShieldCheck, perform: (n) => n("/coverage-lookup") },
  { id: "nav-calendar", label: "去续保日历", group: "导航", icon: CalendarClock, perform: (n) => n("/renewal-calendar") },
  { id: "nav-policies", label: "去保单管理", group: "导航", icon: FileText, perform: (n) => n("/policies") },
  { id: "nav-members", label: "去家庭成员", group: "导航", icon: Users, perform: (n) => n("/members") },
  { id: "nav-insurers", label: "去保险公司", group: "导航", icon: Landmark, perform: (n) => n("/insurers") },
  { id: "nav-assets", label: "去资产管理", group: "导航", icon: Building2, perform: (n) => n("/assets") },
  { id: "nav-visits", label: "去就诊记录", group: "导航", icon: Stethoscope, perform: (n) => n("/medical-visits") },
  { id: "nav-hospitals", label: "去医院管理", group: "导航", icon: Hospital, perform: (n) => n("/hospitals") },
  { id: "nav-doctors", label: "去医生管理", group: "导航", icon: UserRound, perform: (n) => n("/doctors") },
  { id: "nav-settings", label: "去系统设置", group: "导航", icon: Settings, perform: (n) => n("/settings") },
];
...
const NEW_ACTIONS: PaletteCommand[] = [
  ...
  { id: "new-policy", label: "新增保单", hint: "Plus · Policy", group: "快捷操作", icon: Plus,
    perform: (n) => n("/policies?new=1") },
  { id: "new-member", label: "新增家庭成员", hint: "Plus · Member", group: "快捷操作", icon: Plus,
    perform: (n) => n("/members?new=1") },
  { id: "new-visit", label: "新增就诊记录", hint: "Plus · Visit", group: "快捷操作", icon: Plus,
    perform: (n) => n("/medical-visits?new=1") },
  { id: "new-asset", label: "新增资产", hint: "Plus · Asset", group: "快捷操作", icon: Plus,
