...
export function AssetSelector({ assets, selectedAssetId, onSelectAsset }: AssetSelectorProps) {
	...
	if (assets.length === 0) {
		return <div className="text-center text-muted-foreground py-8">暂无资产</div>;
	}
				...
				{assets.map((asset) => {
					...
					return (
						<button
							...
							className={cn(
								"flex-shrink-0 flex flex-col items-center gap-2 p-4 rounded-card transition-all min-w-[140px]",
								isSelected
						...
						>
							...
							<div className="text-center">
								...
								<p className="text-xs text-muted-foreground">{asset.typeLabel}</p>
							</div>
							<div className="text-center">
								<p className="text-xs text-muted-foreground">{asset.activePolicyCount} 份保单</p>
