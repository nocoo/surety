import { ALLOWED_CONTENT_TYPES, MAX_FILE_SIZE } from "@surety/api/lib/attachment-validation";
...
export function AttachmentDropZone({ policyId, onUploadComplete }: AttachmentDropZoneProps) {
				...
				{uploading ? (
					...
					<>
						...
						<p className="mt-2 text-sm text-muted-foreground">
							{isDragActive ? "松开鼠标上传文件" : "拖拽 PDF 或图片到此处，或点击选择文件"}
						</p>
						<p className="text-xs text-muted-foreground/70 mt-1">支持 PDF、JPG、PNG，最大 50MB</p>
					</>
				)}
			</div>
			{error && (
