...
interface T { perStep: Record<string, number>; wall: number; ok: boolean }

async function trial(): Promise<T> {
  const start = performance.now();
  const outcomes = mode === "sequential"
    ? await (async () => { const out = []; for (const s of STEPS) out.push(await runStep(s)); return out; })()
    : await Promise.all(STEPS.map(runStep));
  const wall = performance.now() - start;
  const perStep: Record<string, number> = {};
  let ok = true;
  for (const o of outcomes) { perStep[o.name] = o.ms; if (!o.ok) ok = false; }
...
for (const s of STEPS) bestStep[s.name] = Math.min(...results.map(r => r.perStep[s.name] ?? Infinity));
const bestWall = Math.min(...results.map(r => r.wall));

for (const [name, ms] of Object.entries(bestStep)) console.log(`METRIC ${name.replace(/-/g, "_")}_ms=${Math.round(ms)}`);
