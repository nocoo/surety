...
test("renewal calendar page renders heading and summary cards", async ({
  page,
}) => {
  await page.goto("/renewal-calendar");
  await expect(
    page.getByRole("heading", { level: 1, name: "续保日历" }),
...
test("summary card shows non-empty data from seeded policy", async ({
  page,
}) => {
  await page.goto("/renewal-calendar");
...
test("monthly details section renders", async ({ page }) => {
  await page.goto("/renewal-calendar");
  await expect(
    page.getByRole("heading", { level: 2, name: "月度明细" }),
  ).toBeVisible({ timeout: 10_000 });
});
