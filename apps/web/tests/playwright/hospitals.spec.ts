...
test.describe.serial("hospitals page", () => {
  ...
  test("creates a new hospital", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "添加医院" }),
    ).toBeVisible({ timeout: 10_000 });
  ...
  test("edits an existing hospital", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "编辑医院" }),
    ).toBeVisible({ timeout: 10_000 });
    ...
    await expect(
      page.getByRole("cell", { name: "L3编辑后医院" }),
    ).toBeVisible({ timeout: 10_000 });
  ...
  test("deletes a hospital", async ({ page }) => {
    ...
    await expect(page.getByRole("cell", { name: "L3编辑后医院" })).toHaveCount(
      0,
      { timeout: 10_000 },
    );
  });
