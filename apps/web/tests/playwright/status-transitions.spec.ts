...
async function terminate(request: APIRequestContext, policyId: number): Promise<void> {
...
test.describe("policy-detail status transitions UI", () => {
	test("Active policy: four action entries are visible", async ({ page, request }) => {
		...
		await expect(page.getByRole("button", { name: "退保", exact: true })).toBeVisible();
		await expect(page.getByRole("button", { name: "理赔结案" })).toBeVisible();
		await expect(page.getByRole("button", { name: "标记失效" })).toBeVisible();
		await expect(page.getByRole("button", { name: "标记拟退保" })).toBeVisible();
	});
	...
	test("status row is readonly — no Select / dropdown", async ({ page, request }) => {
		...
		const statusRow = page.locator("div").filter({ hasText: /^状态/ }).first();
		...
		await expect(statusRow.getByRole("combobox", { name: "状态" })).toHaveCount(0);
	});
	...
	test("after terminate: badge + action area switch to terminal branch", async ({
		...
		await expect(page.getByRole("button", { name: "退保", exact: true })).toHaveCount(0);
		await expect(page.getByRole("button", { name: "标记拟退保" })).toHaveCount(0);
		// Terminal branch entries are present.
		await expect(page.getByRole("button", { name: "修改终止信息" })).toBeVisible();
		await expect(page.getByRole("button", { name: "恢复 Active" })).toBeVisible();
	});

	test("terminal state does NOT expose planned-surrender entry", async ({ page, request }) => {
		...
		await expect(page.getByRole("button", { name: "标记拟退保" })).toHaveCount(0);
		await expect(page.getByRole("button", { name: "编辑拟退保" })).toHaveCount(0);
	});
	...
	test("terminated row edit form: status select reduces to Paid only and back-fill succeeds", async ({
		...
		const paymentRes = await request.post(`/api/policies/${policy.id}/payments`, {
			data: {
				periodNumber: 1,
				dueDate: "2026-06-15",
				amount: 1000,
		...
		const terminateRes = await request.post(`/api/policies/${policy.id}/terminate`, {
			data: {
				status: "Surrendered",
		...
		await page.locator("button.size-6:has(svg.lucide-pencil)").first().click({ force: true });
		...
		// order or nth().
		const statusTrigger = page.getByRole("combobox").filter({ hasText: "已缴" });
		await expect(statusTrigger).toBeVisible();
		await statusTrigger.click();
		await expect(page.getByRole("option", { name: "已缴" })).toBeVisible();
