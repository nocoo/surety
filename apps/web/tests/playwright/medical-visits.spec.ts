...
test.describe
	.serial("medical-visits CRUD", () => {
		test("page renders with heading and table", async ({ page }) => {
			await page.goto("/medical-visits");
			await expect(page.getByRole("heading", { level: 1, name: "就诊记录" })).toBeVisible({
				timeout: 10_000,
			});
		...
		test("create a new visit via VisitSheet", async ({ page }) => {
			...
			await expect(page.getByRole("heading", { name: "添加就诊记录" })).toBeVisible({
				timeout: 10_000,
			});
			...
			await expect(page.getByRole("heading", { name: "添加就诊记录" })).toBeHidden({
				timeout: 10_000,
			});
		...
		test("edit the created visit", async ({ page }) => {
			...
			await expect(page.getByRole("heading", { name: "编辑就诊记录" })).toBeVisible({
				timeout: 10_000,
			});
			...
			await expect(page.getByRole("heading", { name: "编辑就诊记录" })).toBeHidden({
				timeout: 10_000,
			});
		...
		test("门诊 and 急诊 use distinct badge variants", async ({ page, request }) => {
			...
			try {
				...
				const menZhenBadge = page.locator('[data-slot="badge"]', { hasText: "门诊" }).first();
				const jiZhenBadge = page.locator('[data-slot="badge"]', { hasText: "急诊" }).first();
