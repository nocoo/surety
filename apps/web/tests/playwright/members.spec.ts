...
test.describe
	.serial("members page CRUD", () => {
		test("renders the members page with header and table", async ({ page }) => {
			await page.goto("/members");
			await expect(page.getByRole("heading", { level: 1, name: "家庭成员" })).toBeVisible({
				timeout: 10_000,
			});
		...
		test("creates a new member", async ({ page }) => {
			...
			await expect(page.getByRole("heading", { name: "添加成员" })).toBeHidden({ timeout: 10_000 });
		...
		test("edits an existing member", async ({ page }) => {
			...
			await expect(page.getByRole("heading", { name: "编辑成员" })).toBeHidden({ timeout: 10_000 });
